...
void kruskal()
{
	...
	for(i=2; i<=m; i++)                                                            //(**)
	{
		if(ddc[i]==0)
		{
			...
			else if(dd[tapcanh[i].dau]!=0&&dd[tapcanh[i].cuoi]!=0&&dd[tapcanh[i].dau]!=dd[tapcanh[i].cuoi])//neu hai dinh cua canh duoc chon thuoc 2 cay khac nhau 
			//thi dua 2 cay do hop thanh 1 cây
			{
				for(j=1; j<=m; j++)
				{
