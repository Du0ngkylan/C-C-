...
int TimSoDoiXungDauTien(int* a, int n)
{
	for(int i = 0; i < n; i++)
	{
		bool checkdoixung = KiemTraSoDoiXung(a[i]);
...
int TimSoCoChuSoDauLeDauTien(int* a, int n)
{
	for(int i = 0; i < n; i++)
	{
		bool checkchusodaule = KiemTraSoCoChuSoDauLe(a[i]);
		if(checkchusodaule)   // <=> if(checkchusodaule == true)
			return a[i];
	}
	return -1;
}
bool KiemTraChuSoCoDang2Mu(int x)
{
	int i = 0;
	...
	{ 
		if(pow(2, i) == x)        // Chứng tỏ x có dạng 2^k
			return true;
		if(pow(2, i) > x)         // ĐK để thoát vòng lặp
			return false;
		i++;
	}
	return false;
...
void LietKeSoCoDang2Mu(int* a, int n)
{
	for(int i = 0; i < n; i++)
	{
		bool checksodang2mu = KiemTraChuSoCoDang2Mu(a[i]);
