 ...
 * n ki tu tiep theo la cac chu so thoa man
 * khong co 2 chu so nao giong nhau
 */
...
//Ham kiem tra xem chu so thu i co trung voi cac chu so dung truoc hay khong
...
void sinhChuSo(int i) {
	if (i == n) { //Neu i = n nghia la ta da sinh du n chu so
		...
		for (chuSo[i] = 0; chuSo[i] <= 9; chuSo[i]++) {
			if (kiemTraHopLe(i) == 1) { //Kiem tra tinh hop le cua gia tri gan cho
										//chu so thu i
				sinhChuSo(i + 1);
			}
		}
